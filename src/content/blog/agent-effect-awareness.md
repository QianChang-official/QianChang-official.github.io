---
title: 朋友拿了个 Agent 安全方案给我看，我看完只说了一句：这玩意有必要存在吗
...
这是我最不舒服的地方。YoloFS（SOSP'26）的思路不是"怎么让 Commit 正确"，是"为什么要在真实文件系统 Commit"——所有修改先放进 staging filesystem，用户看到最终结果再决定合不合并。
...
近两年 Agent 安全的主线是 Capability / Information Flow / Runtime Enforcement / Policy Verification，共同关心的是 **Agent 为什么能做某事**。AgentSpec 管哪些工具允许被调用，Safe Tool Use 管能力传播和信息流风险。

这方案管的是：Agent 被允许做之后，文件对象有没有变。问题突然小了一层，格局也跟着小了。
...
这方案默认的链路是"用户看到 Preview → 批准 → 系统保证 Commit 没被替换"。但现实中 Agent 经常不是这么死的，是死于 **Agent 自己根本不知道它干了什么**。

YoloFS 收集的很多事故其实接近这个：rm 执行了，Agent 说"我已经成功清理缓存"，实际删了用户文件。这时候 E_preview == E_actual 甚至成立，问题还是发生了。
